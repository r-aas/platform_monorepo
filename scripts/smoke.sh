...
http_check() {
  local url="$1" label="$2" expect="${3:-200}" headers="${4:-}"
  CODE=$(curl -sk -o /dev/null -w '%{http_code}' --max-time 5 ${headers:+-H "$headers"} "$url" 2>/dev/null) || CODE=000
  if [ "$CODE" = "$expect" ]; then
    ok "$label ($CODE)"
...
app_exists "genai-odd-platform" && \
  http_check "http://odd.platform.127.0.0.1.nip.io/actuator/health" "ODD Platform"
echo ""
