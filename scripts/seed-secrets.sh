...
# Read password from the pgvector secret (auto-generated by Bitnami chart)
_PGV_PASS=$(kubectl get secret genai-pgvector -n genai -o jsonpath='{.data.password}' 2>/dev/null | base64 -d || echo "${PGVECTOR_PASSWORD:-postgres}")
create_secret genai odd-platform-db \
  --from-literal=password="${_PGV_PASS}"
